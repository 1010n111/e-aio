...
> **上游文档**：[[01-企业级一体化管理系统-e-aio-可行性研究报告|01-可行性研究报告]]、[[02-企业级一体化管理系统-e-aio-软件需求规格说明书|02-软件需求规格说明书（SRS）]]
> **版本**：V1.1（审核修订）
> **日期**：2026-09-16
...
6. [[01-企业级一体化管理系统-e-aio-可行性研究报告|01-可行性研究报告]]
7. [[02-企业级一体化管理系统-e-aio-软件需求规格说明书|02-软件需求规格说明书（SRS）]]
...
| 平台底座 | report（报表 BI） | 3.16 FR-RPT |
| 平台底座 | ai（AI 能力） | 3.19 FR-AI |
| 平台底座 | platform（基础平台：参数/字典/文件/定时任务/Excel/缓存/监测） | 3.22 FR-PLT |
| 通用业务 | oa（OA 协同） | 3.6 FR-OA |
| 通用业务 | crm（客户关系） | 3.7 FR-CRM |
| 通用业务 | inventory（库存） | 3.8 FR-INV |
| 通用业务 | finance（财务） | 3.9 FR-FIN |
| 通用业务 | hr（人力资源） | 3.10 FR-HR |
| 通用业务 | project（项目管理） | 3.11 FR-PROJ |
| 通用业务 | scm（供应链） | 3.12 FR-SCM |
| 通用业务 | marketing（营销） | 3.13 FR-MKT |
| 通用业务 | service（售后客服） | 3.14 FR-SRV |
| 通用业务 | fund（资金管理） | 3.15 FR-FUND |
| 平台支撑 | portal（门户消息） | 3.17 FR-PORTAL |
| 平台支撑 | integration（开放平台集成） | 3.18 FR-INT |
| 平台支撑 | mobile（移动端） | 3.20 FR-MOB |
| 平台支撑 | i18n（多语言多币种） | 3.21 FR-I18N |

> **说明**：SRS 中"人力资源 HRM"的组织架构能力（FR-HR-06/07/08）划入 `iam` 模块统一支撑权限与通讯录，`hr` 模块承载考勤/薪酬/招聘/绩效等事务能力；两模块通过公共 API 协作。
...
- RuoYi-Vue / RuoYi-Vue3 采用 **MIT** 许可证；P0 起采取 **clean-room 参考重写**：不拷贝源码，`NOTICE` 记上游许可证名与蓝本 tag/commit 作溯源即可合规（e-aio 自身为 Apache-2.0）。若后续某个模块确需直接复用上游源码，则必须保留其版权声明与 LICENSE 文本——该判断在引入当次评审中作出。
...
- **错误码规范**：统一数值码（`0` 成功；通用段 `10000–19999`：参数校验/数据不存在/未认证/无权限/系统错误/重复提交等；业务段 `20000+` 按模块前缀映射分配）；错误码表在详细设计附录维护（见 P0 分册 3.2.3 / 6.1）。
