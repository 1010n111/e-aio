...
**两级**：Caffeine 本地（`eaio.cache.local.enabled` 默认 true，TTL 60s，`maximumSize` 10000）+ Redis（`eaio.cache.redis.ttl-seconds` 默认 30 分钟，与 3.15 登记表一致）。**键规范**：`eaio:{env}:cache:{namespace}:{key}`，由 `com.eaio.common.redis.RedisKeys` 统一构造（禁止散落拼串）。
...
| `eaio.dict.cache-ttl-seconds` | 60 | 字典本地缓存 TTL（与 P1-3 册 3.6.1 区域表一致；T5 落地时同步） |
