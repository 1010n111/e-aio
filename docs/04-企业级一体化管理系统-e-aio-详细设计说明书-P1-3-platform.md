...
**实现注记（T8 已落地，2026-09-19）**——落地时与本册口径的差异/补充，逐条登记（评审按此对照）：

| # | 本册口径 | 落地实现 | 理由 |
|---|---|---|---|
| 1 | 4.3.14 的 `event_delivery` DDL | `V7__event_delivery.sql` **逐列/逐索引/逐 COMMENT 一致**（含脚本头的反向 SQL 与"数据不可逆点"说明）；4.3 的例外清单也一致——有 `updated_*`/`version`，**没有** `deleted` | DDL 是冻结件：够用就不要改。投递记录的"删除"语义由 `status` 表达（`DONE` 按保留天数物理清理、`DEAD` 保留至人工处置），再加一列 `deleted` 只会让两个真相打架 |
| 2 | 3.9.2 第 1 条"登记与业务同事务" | `PlatformEventPublisher.publish` 标 **`@Transactional`（默认 REQUIRED）**：有业务事务就加入（回滚一起回滚），没有就自己开一个短事务；事务内发布的是 `DeliveryRegistration` **信封**（不是事件本身），事件对象在**提交后**由投递器发布给全部监听方 | `REQUIRES_NEW` 会让业务回滚后登记照样提交 = "发出一个业务上不存在的事件"，本册明确否决。而 `JobExecutor` 发 `JobFailedEvent` 时**没有**事务（终态写定后发布），REQUIRED 的"没有就自己开一个"正好让两条路径共用一份登记逻辑且 `AFTER_COMMIT` 照常触发。**信封这一层是必须的**：5.5 的消费契约是**同步** `@EventListener`，事务内直接发事件会在业务事务里就把监听方跑一遍——回滚后监听方已经执行过（违反第 1 条），提交后投递器还会再投一次（一次业务写投两遍）；信封只有 `PlatformEventDispatcher` 认，监听方拿不到它（`EventRetryIT#rollbackLeavesNoDeliveryRowAndNoListenerCall` 钉住：回滚后登记无行 **且** 监听方一次都没被调用） |
| 3 | 3.9.1"事件 record 前两字段固定 `eventId`/`occurredAt`" | 额外落成**类型化载体** `events/PlatformEvent`（`eventId()`/`occurredAt()`）；`ParamChangedEvent`/`DictChangedEvent`/`JobFailedEvent` 只加 `implements`，**字段与位次一字未改**（`ArchitectureTest#eventRecordsHaveEventIdFirst` 仍绿） | 发件箱要拿 `eventId` 做幂等键、`occurredAt` 做留痕，重投扫描器要按 `eventId` 回写状态：没有接口就只能反射取 record 组件或收 `Object` 到运行期才发现类型不对。约定（字段位次）与类型（接口）是同一件事的两种表达 |
| 4 | 3.9.3"`payload_json` 反序列化失败（事件类被重命名/删除）→ 记 ERROR + 置 DEAD" | `event_type` 写**全限定类名**；重投时经 `events/EventTypes` 的**代码注册表**解析，未登记 → ERROR + DEAD；`publish` 时先校验注册，未登记直接 `SystemException`（业务事务回滚） | **不按 DB 字符串 `Class.forName`**：那是 3.4.1 已否决的"DB 字符串 → 代码路径"（能写表就能触发任意类的加载），可执行类型只能来自代码登记。登记点前移到 `publish` 是为了把"首次投递成功、重投永远解不开 → 静默死信"挡在开发/测试期；`EventTypesTest` 扫描 events 包，新增事件漏登记即红 |
| 5 | 3.9.2 第 2 条"任一监听器抛异常 → UPDATE attempt=1→2 …"（未说异常往哪去） | 异常在 `PlatformEventDispatcher` 内**全部消化、绝不外抛**（记 ERROR + 写状态）；死信事实的发布也单独 try/catch | 回调跑在发布方线程里：异常冒出去就是"库已经提交、接口却报错"，比失败本身更难排查。**状态必须落库**才是这条口径的落点 |
| 6 | 3.9.2 第 3 条"`next_retry_time = now + backoff × 2^(attempt-1)`，封顶 30min" | 退避**复用** `domain/job/JobRetryPolicy.nextRetryAt/backoff`（同公式、同 30 分钟封顶、同溢出防护）；事件侧的差异只有上限语义 | 同一条公式抄第二遍 = 两处要修（尤其 `1L << exponent` 的溢出防护）。上限语义不同：`job.retry_max` 是"可重试次数"（`attempt ≤ retry_max`），`event_delivery.max_attempt` 是**最大投递次数**（`attempt + 1 > max_attempt` 即 DEAD，默认 5 → 最多投 5 次），因此不套用 `canRetry` |
| 7 | 3.9.2 第 4 条"死信写 `alert`（`rule_code = event.dead-letter`）" | **未做到**（登记承接方）：只发 `events/EventDeadLetteredEvent`（record，前两字段 `eventId`/`occurredAt`，另带 `originalEventId`/`eventType`/`attemptCount`/`lastError`/`traceId`）+ 记 ERROR | `alert`/`alert_rule` 属 V9 与告警能力的票（T13），本票既没有表也没有 `NoticeApi`——与 T7 对 `JobFailedEvent` 的处理同一先例。**承接方：告警能力**（订阅 `EventDeadLetteredEvent`，落 `rule_code = event.dead-letter`） |
| 8 | 3.9.2 第 2/4 条都没说死信事实自己怎么投 | `EventDeadLetteredEvent` 实现 `events/InternalFactEvent`：**不进发件箱**（没有登记行，投递失败只记 ERROR、不重试），dispatcher 对内部事实把"无登记行"从 WARN 降为 DEBUG | 死信事实是在投递回调里产生的**结果**：把它也登记一次会造出"死信的死信"链条（每个死信事实失败又变成一条死信），而它的价值恰恰是尽快送达。`alert` 能力订阅它即可，不依赖发件箱的重试语义 |
| 9 | 3.9.2 第 3 条"重投在 `platform.event.retry` 任务里执行" | `EventRetryScanner implements JobHandler`，`code() = platform.event.retry`——**核对种子后的结论：R__platform_seed.sql 的 ID 54 就是 `platform.event.retry`（`*/30 * * * * *`、`retry_max = 0`，且 `handler_code` 与 `job_code` 逐字相同），无需改种子、无需新建任务行**；T7 种下的任务行到本票才真正有处理点 | 注册表（`JobHandlerRegistry`）按 Bean 收集可执行点，种子只种"任务元数据"：两票合起来才是完整的"任务可跑"（3.4.5 的清单先种行、处理点随能力的票注册） |
| 10 | 3.9.4 第 5 条"两实例并发扫描 → 同一条记录只重投一次（ShedLock）" | 扫描器自己取一把锁：`platform-internal-event-retry-scan`（`JobLockWindow.INTERNAL_LOCK_PREFIX + "event-retry-scan"`），**持锁上限 120 秒** | 任务框架那把锁（`platform-job-{jobCode}`）只保证"同一任务不被两个实例同时跑"，而本处理点还会被测试/运维**直接调用**（`scanOnce()`）；锁放在扫描器里才覆盖全部入口。命名空间前缀保证与任何任务锁、与 T7 的内部重投扫描锁 `platform-internal-retry-scan` 都不撞名。120 秒 ≫ 正常一轮（≤100 条、监听方 <100ms），实例真宕机最多 2 分钟后可接管 |
| 11 | 3.9.3"`DONE` 保留 7 天后由 `platform.event.retry` 顺带清理；`DEAD` 保留至人工处理" | 同一个处理点收尾时分批物理删除（1000 行/批、单次最多 100 批）；删除线 `coalesce(finish_time, created_at) < now - retainDays`，`WHERE status = 'DONE'` | 分批避免一次 DELETE 撑爆 WAL（3.4.4 同款）；`coalesce` 是防"手工改库留下 `finish_time IS NULL` 的 DONE 行永远删不掉"（静默堆积）。`DEAD` 不在删除条件里，构造上不可能被自动删 |
| 12 | 3.9.2 第 3 条的批量大小：7.2 **没有**事件侧的批量参数键 | 代码常量：`RETRY_BATCH_SIZE = 100`（一轮最多重投 100 条，跑不完下一轮继续） | 不新增参数键：7.2 是参数键的唯一来源，T1 已冻结段内号位；批量大小不是运维需要热调的量（30 秒一轮足够吞吐） |
| 13 | 5.5 消费契约"**同步 `@EventListener`**（在发布方 AFTER_COMMIT 回调线程内执行）" | `ParamInvalidationListener`/`DictInvalidationListener` 从 `@TransactionalEventListener(AFTER_COMMIT)` **改为普通 `@EventListener`**（T3/T4/T5 交付的既有监听器）；"提交后才投递"由 `PlatformEventDispatcher` 保证 | 挂 AFTER_COMMIT 时**重投与人工重放路径没有事务**，监听器会被静默跳过：投递记录写成 `DONE`、缓存却没清——最坏的一类"状态显示成功、语义没执行"。语义没有变化：事务回滚时 dispatcher 根本不投递，"回滚了就不清缓存"仍成立（`EventRetryIT#retryRedeliversToRealInvalidationListenerAndClearsL1` 用真实 L1 读到新值证明重投真的打到了监听方） |
| 14 | 5.2 的 `{eventId}` 只给字段不给类型 | 落成具名 record `EventIdCmd`（`Get` 与 `Replay` 共用）；出参见 5.3 的 `EventDeliveryDTO` | 同 T3/T5/T7 的先例（5.2 给字段不给类型名）；两个端点的入参逐字相同，拆两个 record 只会多一处抄写 |
| 15 | 5.3 的 `payloadPreview`(≤1000 字符) | 是**派生值**：`payload_json` 的前 1000 字符（超长截断、不加省略号）；不是表里的列 | 4.3.14 没有 `payload_preview` 列（DDL 冻结），且"管理页只看预览"是安全口径（避免管理页成为数据外泄面），由 DTO 层做最合适 |
| 16 | 7.2 的 `platform.event.*` 三个键 | 种子追加 ID **61–63**（`platform.event.retry-max`=5、`platform.event.retry-backoff-seconds`=30、`platform.event.delivery-retain-days`=7，`value_type = INT`、`hot_reload = true`、`builtin = true`、`ON CONFLICT DO NOTHING`）；`param_group = 'event'` | 7.2 的表只有"键/默认值/类型/热更新/说明"五列，没有分组列，而 4.3.1 的 `param_group` 是 NOT NULL：按既有先例（time/file/excel）取能力名。ID 从 61 起是"不碰参数 1–10、字典 21–45、任务 51–56"的固定字面量区间 |
| 17 | 3.9.2 未说登记行的 `created_by` 从哪来 | 留空（`NULL`）；操作者留痕仍由业务域自己的列承担（如 `param_change_log.operator_id`） | 发布口不持有操作者上下文（重投扫描线程上更没有）；从事件表再抄一遍操作者只会多一个会过期的副本 |
| 18 | 3.9.4 的测试点（7 条） | 单测 41 条：`PlatformEventPublisherTest`（登记字段/事务传播 REQUIRED/信封而非事件本身/未登记类型拒发）、`PlatformEventDispatcherTest`（信封触发器、成功→DONE、失败→attempt=2+退避、封顶 30min、超限→DEAD+死信事实、死信发布失败不外抛、内部事实只广播）、`EventRetryScannerTest`（编码=种子任务码、逐行重投、拿不到锁一行不碰、锁名命名空间、未登记类型→DEAD、坏载荷→DEAD、超上限→DEAD、超时退出、分批清理 DONE、无库跳过）、`EventDeliveryDtoMapperTest`（预览截断 1000）、`EventDeliveryAppServiceTest`（20070/20071/重放重置 attempt + 历史保留）、`EventDeliveryControllerTest`（路由/出参/权限点字符串逐字）、`EventTypesTest`（事件包登记完整性）；IT `EventRetryIT` **8 条**（回滚无行且监听方未被调用、失败→重投→DONE+finish_time、超限→DEAD→真实 HTTP 重放→RETRYING/attempt=1/历史保留→重投→DONE、死信端点 20070、重复投递一次副作用、两上下文并发只重投一次、重投真的清掉 L1（读到新值）、超期 DONE 被删而 DEAD 保留） | 6.2 的 `EventRetryIT` 用真 PG17 + 真 Redis + 真 HTTP 实现；**"两实例并发"用 `SpringApplicationBuilder` 起第二个上下文**（同 DB/Redis，`WebApplicationType.NONE`，用完显式 `close()`，与 `SchedulerLockIT` 同一手法）。后台每 30 秒的调度器是生产行为，因此断言写在**终态**上（谁投的都一样，不能变的是"同一条只投一次"） |
| 19 | 4.5 的迁移表：`V7__event_delivery.sql` | 落地 `V7__event_delivery.sql`；V4/V6 在本票时点仍不存在（属文件中心/Excel 的票），Flyway 允许跳号 | `PlatformMigrationIT` 的期望值从**迁移目录推导**（脚本条数、最高版本号、表清单都现算），新增一票迁移不需要手改它 |
