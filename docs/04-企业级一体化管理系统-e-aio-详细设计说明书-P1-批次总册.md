...
8. **代价已显式登记**：审计异步写入存在崩溃窗口（以 STRONG 模式与补偿缓解，audit 册 3.3）；数据权限 SQL 改写带来查询复杂度与索引强依赖（iam 册 3.6）；参数/权限快照缓存带来一致性延迟（以变更事件失效，iam 册 4.7）。
...
| 6 | 幂等 `fail-closed`（`10502`）与长任务 `taskId` 口径 | 长任务（Excel 导入导出、审计举证导出、归档）一律 `taskId` + 轮询 | platform 册 3.5、audit 册 3.8 |
...
| 3 | 表名 `snake_case` 复数、显式 schema 限定、索引命名 `idx_<table>_<cols>` | DDL 通读 |
...
| 4 | 数据权限 SQL 改写方案 | iam 册 3.6（本批次交付） |
