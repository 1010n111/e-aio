...
- 所有请求走 `src/utils/request.js` 统一封装：POST + JSON、响应拦截。认证/鉴权失败按 `Result.code`（`10401` / `10403`）跳登录——**后端 HTTP 恒 200，不要按 401/403 判断**。
...
- 目录：`src/api/`、`src/utils/request.js`、`src/views/`（P1 起按模块挂载）；`vite.config.js` 开发代理 → 后端，生产独立部署。
